...
function normaliser(brut: Record<string, unknown> | null | undefined): Answers {
  ...
  for (const [id, valeur] of Object.entries(brut ?? {})) {
    if (Array.isArray(valeur))
      sortie[id] = valeur.filter((v): v is string => typeof v === 'string');
...
export const useCertificationStore = defineStore('certification', () => {
  ...
  async function submit(): Promise<string | null> {
    ...
    error.value = null;
    try {
      const res = await CertificationService.updateAttempt(
        attemptId.value,
        answers.value,
        'submitted',
      );
  ...
  /*
   * Leaving the screen and coming back must land on the choice, not in the
   * middle of the last test. Only the in-memory session is cleared: the
   * remembered attempt stays, so « Reprendre » is still offered.
   */
  function revenirAuChoix(): void {
