...
defineProps<{ profils: ProfilResume[], premierVue : number}>();
</script>

<template>
  <!--
    Grille 3 colonnes de la maquette, contre une rangée qui s'enroule côté
  ...
  <ul class="grid list-none grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-8">
    <li v-for="profil in profils.slice(premierVue).slice(0 , 20)" :key="profil.id || profil.nom" class="flex">
